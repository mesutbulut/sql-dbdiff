...
namespace DBDiff.Schema.SQLServer.Generates.Generates
{
    public class GenerateTables
    {
        ...
        private void FillColumn<T>(ITable<T> table, SqlDataReader reader) where T:ISchemaBase
        {
            ...
            if (((Database)table.Parent).Info.Version == DatabaseInfo.VersionNumber.SQLServer2008)
            {
                        ...
                        while (reader.Read())
                        {
                            ...
                            if (lastObjectId != (int)reader[TableIdIndex])
                            {
                                ...
                                if (isTable)
                                {
                                    ...
                                    if ((database.Options.Ignore.FilterTableLockEscalation) && (database.Info.Version == DatabaseInfo.VersionNumber.SQLServer2008))
